...
export default function AssetDetailPage() {
  ...
  return (
    <div>
      ...
      {/* 3D Digital Twin preview — embeds standalone Three.js viewer for the active twin.
          Selection priority:
            1. Missions whose status indicates ODM has run (processing_3d / completed)
            2. Tie-break: oldest among those (so the canonical / original twin stays pinned).
          This stops fresh "planned" missions (just-uploaded photos with no twin yet) from
          replacing a known-good twin in the viewer slot. */}
      {missions.length > 0 && (() => {
        ...
        const latest = (withTwin.length > 0 ? sortByOldest(withTwin)[0] : missions[0]);
        const twinUrl = `/twin/?mid=${latest.id}&name=${encodeURIComponent(asset.name)}`;
        const processing = latest.odm_status && latest.odm_status !== 'completed' && latest.odm_status !== 'failed';
        return (
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-card mb-6">
            ...
            <p className="text-sm text-slate-400 mb-3">
              Latest reconstruction from mission &quot;{latest.name}&quot;.
              {processing && (
            ...
            <div className="w-full rounded-lg overflow-hidden border border-slate-200 bg-slate-900" style={{ height: 600 }}>
              <iframe src={twinUrl} className="w-full h-full block" title={`3D Twin — ${asset.name}`} />
            </div>
          </div>
        );
      })()}
